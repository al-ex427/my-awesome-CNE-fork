...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		{
			var menuItem:FlxSprite = new FlxSprite(25, 60 + (i * 160));
		...
		//	menuItem.screenCenter(X);
			...
			menuItem.scrollFactor.set(0, 0.125);
			menuItem.antialiasing = true;
		}

		FlxG.camera.follow(camFollow, null, 0.06);

		versionText = new FunkinText(640, 5, 640, 'Awesome CNE Fork (v${Application.current.meta.get('version')})', 20);
		#if MOD_SUPPORT
		versionText.text += '\n[${controls.getKeyName(SWITCHMOD)}] - Open Mod Switcher';
		#end
		if (canAccessDebugMenus) {
			versionText.text += '\n[7] - Debug Tools\n';
		}
	//	versionText.x -= versionText.width + 5; 
	versionText.alignment = RIGHT;

		versionText.setBorderStyle(FlxTextBorderStyle.OUTLINE, FlxColor.BLACK, 2, 5);
